...
@router.get(
    "/rules",
    response_model=list[DTOS.RuleDTO],
...
async def list_rules(service: RuleService = Depends(get_service)):
    ...
    return [DTOS.RuleDTO.model_validate(r.model_dump(by_alias=True)) for r in rules]


@router.get(
    "/rules/{rule_id}",
    response_model=DTOS.RuleDTO,
    status_code=status.HTTP_200_OK,
...
async def get_rule(rule_id: str, service: RuleService = Depends(get_service)):
    ...
    return DTOS.RuleDTO.model_validate(rule.model_dump(by_alias=True))


@router.put(
    "/rules/{rule_id}",
    status_code=status.HTTP_200_OK,
    response_model=DTOS.MessageDTO,
...
async def update_rule(rule_id: str, data: RuleModel, service: RuleService = Depends(get_service)):
    ...
    return DTOS.MessageDTO(message="Rule updated")
